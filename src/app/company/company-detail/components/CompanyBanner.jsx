...
const CompanyBanner = () => {
  ...
  return (
    <div className="relative h-64 mx-auto overflow-hidden bg-center bg-cover rounded-lg shadow max-w-7xl"
      style={{
        backgroundImage: `url(${company.cover_photo || 'https://static.topcv.vn/company_covers/tap-doan-cong-nghiep-vien-thong-quan-doi-e3c6e7727df189e29507b150c6a7d893-64c328ef424bd.jpg'})`
      }}>
      {/* Overlay nền đậm mờ */}
      <div className="absolute bottom-0 left-0 flex items-center justify-between w-full px-4 py-4 border rounded-b-lg bg-white/20 backdrop-blur-sm border-white/30">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <div className="mt-2 text-sm text-gray-600">
              ...
              <a href={company.website} className="hover:text-blue-600 underline">{company.website?.replace('https://', '')}</a>
              · {company.quantity_employee}+ nhân viên · {Math.floor(Math.random() * 500)} người theo dõi
            </div>
          </div>
        </div>
        {/* Nút theo dõi */}
        ...
        >
          {company.isFollowing ? '✓ Đang theo dõi' : '+ Theo dõi công ty'}
        </button>
      </div>
    </div>
  );
