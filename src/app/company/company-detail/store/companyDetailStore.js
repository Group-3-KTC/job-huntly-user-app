...
const useCompanyDetailStore = create((set, get) => ({
  ...
  fetchCompanyDetail: async (companyId) => {
    ...
    try {
      ...
      const response = await fetch(`https://68808ec2f1dcae717b627e5b.mockapi.io/companies/${companyId}`);
  ...
  fetchCompanyJobs: async (companyId) => {
    try {
      // Giả lập dữ liệu việc làm vì API thực tế không có
      // Trong thực tế, bạn sẽ gọi API để lấy danh sách việc làm theo company_id
      const mockJobs = [
        {
          id: "1",
  ...
  toggleFollowCompany: () => {
    const company = get().company;
    if (company) {
      const updatedCompany = { ...company, isFollowing: !company.isFollowing };
      set({ company: updatedCompany });
      
      // Trong thực tế, bạn sẽ gọi API để cập nhật trạng thái theo dõi
      console.log(`${updatedCompany.isFollowing ? 'Đã theo dõi' : 'Đã hủy theo dõi'} công ty: ${company.name}`);
    }
  }
}));
